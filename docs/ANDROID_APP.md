...
| `mobile/app/.../auth/FirebaseAuthClient.kt` | Production `AuthClient`: Credential Manager account picker (`GetGoogleIdOption` with the `default_web_client_id` generated by the google-services plugin) → `GoogleIdTokenCredential` → `FirebaseAuth.signInWithCredential`, plus email/password sign-in/sign-up (`signInWithEmailAndPassword` / `createUserWithEmailAndPassword`, SDK failures mapped to `EmailAuthException`) → Firebase ID token as the bearer; session persisted by Firebase Auth itself; dismissed picker → `SignInCancelledException`; sign-out also clears Credential Manager state |
...
| `mobile/shared/domain/.../auth/AuthClient.kt` | `AuthClient` port (signIn/signInWithEmail/signUpWithEmail/currentToken/signOut + `authState`); production flow: Credential Manager (Google) or email/password → Firebase Auth SDK → Firebase ID token as the bearer; `AuthState` (Unknown/SignedOut/SignedIn), `AuthUser` (uid + email + display name), `SignInCancelledException`, `EmailAuthException` (`INVALID_CREDENTIALS` / `EMAIL_ALREADY_IN_USE` / `WEAK_PASSWORD`) |
