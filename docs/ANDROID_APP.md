...
Firebase Analytics and Crashlytics are deny-by-default in the manifest and reset off at app start.
The current per-user legal/privacy acceptance enables them; sign-out disables them. Analytics uses
only fixed screen/workflow enums with no identifiers or free text. Custom and uncaught failures
preserve stack frames but replace messages/causes before Crashlytics receives them.
...
│       ├── telemetry/                   # consent gate + Analytics/Crashlytics adapter
│       └── ui/
│           ├── navigation/AppNavHost.kt        # record / history / assessment/{caseId}
│           ├── navigation/AuthGate.kt          # login gate → versioned legal gate → AppNavHost
│           ├── legal/                          # Terms/Privacy dialogs + acceptance screen
│           ├── login/LoginScreen.kt            # auth, password reset, Terms/Privacy links
...
└── shared/
    ...
    │   │   ├── legal/SharedPreferencesLegalAcceptanceStore.kt # per-user legal acceptance
...
| `mobile/app/.../di/AppModule.kt` | Koin bindings for Firebase/fake auth, SQLDelight case/result/queue persistence, WorkManager assessment processing, versioned legal acceptance, backend API, use cases, and ViewModels |
| `mobile/app/.../telemetry/AppTelemetry.kt` | Consent-gated Firebase Analytics/Crashlytics adapter; fixed PHI-free events, collection reset, and sanitized fatal/non-fatal exceptions |
| `mobile/app/.../MainActivity.kt` | Sets Compose content; hosts `AuthGate` inside `Scaffold` |
| `mobile/app/.../ui/navigation/AuthGate.kt` | Observes auth state; signed-out users see login while signed-in users pass through a per-user, versioned `LegalGate` before app navigation |
| `mobile/app/.../ui/legal/*` | Explicit legal acknowledgement gate, persistent Terms/Privacy links, CDS/emergency/consent/data warnings, and draft legal-review notice |
| `mobile/app/.../ui/login/LoginScreen.kt` | Google/email auth, account creation, password reset, localized state, and Terms/Privacy links |
